...
"""
Acceptance test (a) for prompt U3
(.prompts/gradient-coupled-plotting/04-U3-persist-and-rehydrate-parity-columns.md).

Stores a GradientCoupledInstanton with a full computed result (including the
eleven new parity scalars: C_peak, C_bar_peak, C_min, compensated, type_II,
r_max, r_peak, M_max, M_peak, V_end_downflow, N_end_downflow), reloads it via
build() with the default do_not_populate=False, and asserts every one of the
eleven properties round-trips to the pre-store value.

Follows the same live_pool integration pattern already used by
tests/test_gradient_coupled_instanton_end_to_end.py's own
TestGradientCoupledInstantonPersistence class -- a fake trajectory stand-in
(store_id + units only, mirroring that file's _make_fake_traj) is stored
against real, pool-minted N_init/N_final/delta_Nstar/tolerance/
n_collocation_points/alpha_regularization/diffusion/cosmo prerequisites, and
the parity scalars are set directly on the object (mirroring that file's own
_make_gci helper, which sets msr_action/noise_* the same way) rather than
run through a genuine Picard solve -- this is a persistence round-trip test,
not a numerical-physics correctness test, so deterministic, hand-chosen
values are used to avoid flakiness near the C_threshold gate that can make
r_max/M_max genuinely None in a real solve (see test_gci_parity_scalars.py's
own "if ... is not None" guards for that same reason).
"""
...
# The eleven parity scalars under test, in working (reduced-Planck) units --
# see the module docstring for why these are hand-chosen rather than solved.
_PARITY_SCALARS = dict(
    ...
    M_max=3.1e-2,
    M_peak=3.3e-2,
    V_end_downflow=8.7e-9,
    N_end_downflow=0.037,
)
...
def test_gci_parity_scalars_round_trip_through_full_fidelity_store(live_pool):
    prereqs = _get_prerequisites(live_pool)
    n_init = _mint_n_init(live_pool, _GCI_N_INIT_ROUNDTRIP)
    ...
    # Sanity: this is a full-fidelity fetch, not the cheap tier under test
    # in test_gci_parity_cheap_fetch.py.
    assert len(gci_read.values) == len(_N_SAMPLES)
    assert len(gci_read.profile) == _N_COLLOC_VAL

    # Dimensionless / no-conversion scalars round-trip exactly.
    assert gci_read.C_peak == gci.C_peak
    assert gci_read.C_bar_peak == gci.C_bar_peak
    assert gci_read.C_min == gci.C_min
    assert gci_read.compensated is gci.compensated
    assert gci_read.type_II is gci.type_II
    assert gci_read.N_end_downflow == gci.N_end_downflow

    # Unit-converted scalars (r_max/r_peak via Mpc, M_max/M_peak via
    # SolarMass, V_end_downflow via PlanckMass**4) round-trip to float64
    # precision, not bitwise -- the store()/build() pair divides then
    # multiplies by the same (large) unit constant.
    assert gci_read.r_max == pytest.approx(gci.r_max, rel=1e-12)
    assert gci_read.r_peak == pytest.approx(gci.r_peak, rel=1e-12)
    assert gci_read.M_max == pytest.approx(gci.M_max, rel=1e-12)
    assert gci_read.M_peak == pytest.approx(gci.M_peak, rel=1e-12)
    assert gci_read.V_end_downflow == pytest.approx(gci.V_end_downflow, rel=1e-12)

    # The one child-table column this prompt also had to add (C_bar on
    # GradientCoupledInstantonProfile) -- round-trips alongside zeta/r_ratio/C.
    for orig, restored in zip(gci._profile, gci_read.profile):
        assert restored.C_bar == pytest.approx(orig.C_bar, rel=1e-10)
