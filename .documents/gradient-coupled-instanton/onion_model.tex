...
\begin{equation}
    ...
    =
    \int \Dm{\hat\field} \,
    \exp\!\left[\im\!\int_{\Ninit}^{\Nfinal}\!\!\d\Ny\int_{-1}^1\!\measure\,\d\yco\;
...
\eqref{eq:noise-corr} produces the weight $\e{-\mathcal S}$ with
\begin{equation}
    \mathcal S
    =
    -\im\!\int\!\big(\hat\field(\dot\field-\mom-\advcoef\partial_\yco\field)
    + \hat\mom(\ldots)\big)
    +
    \tfrac12\!\int\!\big(\Dnoise{\field}\hat\field^2 + 2\Dnoise{\field\mom}\hat\field\hat\mom + \Dnoise{\mom}\hat\mom^2\big) ,
\end{equation}
...
saddle-point equations real, and $\rfield,\rmom$ --- \emph{not}
$\hat\field,\hat\mom$ --- are the fields appearing everywhere below and in
the numerical implementation (\texttt{FullInstanton}'s $P_1,P_2$).
...
quadratic term into
$-\tfrac12(\Dnoise{\field}\rfield^2+2\Dnoise{\field\mom}\rfield\rmom+\Dnoise{\mom}\rmom^2)$
...
\begin{align}
    ...
    + \rfield\,(1-\epscore)\left[\frac{1}{\Deltas}-\frac32\right]
    ...
    + \rmom\,(1-\epscore)\left[\frac{1}{\Deltas}-\frac32\right]
...
\eqref{eq:advection-operator}).

\begin{calculation}
\semibold{Response-field equations: direct re-derivation.} Varying
\eqref{eq:msr-action} with respect to $\field$ requires two integrations
by parts: in $\Ny$ for the $\rfield\dot\field$ term (picking up a measure
derivative, since $\measure$ depends on $\Ny$ explicitly through
$\Deltas(\Ny)$, not only through $\field,\mom$), and in $\yco$ for the
advection and gradient terms (using $\Lop$'s self-adjointness). Term by
term:
\begin{itemize}
    \item $\rfield\dot\field$: $\int\d\Ny\,\measure\,\rfield\dot\field
        \to -\partial_\Ny(\measure\,\rfield) = -\dot\measure\,\rfield-\measure\,\dot{\rfield}$;
    \item $-\advcoef\,\rfield\,\partial_\yco\field$: integrating by parts
        in $\yco$ gives $+\partial_\yco(\measure\,\advcoef\,\rfield)$;
    \item $\rmom\,\Vpot'(\field)/\Hub^2$: gives $\measure\,\rmom\,\Vpot''(\field)/\Hub^2$ directly;
    \item $-\rmom\,\Lop\field/(\rout^2\aHloc^2)$: self-adjointness of $\Lop$
        moves the operator onto $\rmom$, giving $-\measure\,\Lop\rmom/(\rout^2\aHloc^2)$.
\end{itemize}
Setting the sum to zero and dividing by $\measure$,
\begin{equation}
    \dot{\rfield}
    =
    -\frac{\dot\measure}{\measure}\rfield
    + \frac{1}{\measure}\partial_\yco\big(\measure\,\advcoef\,\rfield\big)
    + \frac{\Vpot''(\field)}{\Hub^2\atloc}\rmom
    - \frac{1}{\rout^2\aHloc^2}\Lop\rmom ,
\end{equation}
and the analogous variation with respect to $\mom$ gives
\begin{equation}
    \dot{\rmom}
    =
    -\frac{\dot\measure}{\measure}\rmom
    + \frac{1}{\measure}\partial_\yco\big(\measure\,\advcoef\,\rmom\big)
    - \rfield + \big(3-\epsSR\atloc\big)\rmom .
\end{equation}
\emph{Every} term here is opposite in sign to an earlier pass at these
equations (advection/adjoint term, potential/gradient term, and the
$\rfield\leftrightarrow\rmom$ coupling term were all flipped), and the
$-\dot\measure/\measure\,X$ term was missing entirely --- an artefact of
$\measure(\yco,\Ny)=\e{-\frac32\Deltas(\Ny)\yco}$ depending on $\Ny$
explicitly, which vanishes in the 1D limit ($\advcoef=0$, no
$\yco$-dependence) and so was never exercised by the existing
single-trajectory code.

\semibold{Cross-check against \texttt{FullInstanton}.} In the 1D limit
($\advcoef=0$, $\dot\measure/\measure=0$) these reduce to
$\dot{\rfield}=\Vpot''(\field)\rmom/\Hub^2$,
$\dot{\rmom}=-\rfield+(3-\epsSR)\rmom$, which is exactly what
\texttt{FullInstanton.py}'s \texttt{bwd\_rhs} already implements
(\texttt{P2 * d2V\_dphi2(phi1) / Hsq}, \texttt{-P1 + (3-eps)*P2}) --- an
independent, already-exercised confirmation of the corrected sign.

\semibold{Expanding the adjoint term and combining with the measure
derivative.} Using $\d\Deltas(\Ny)/\d\Ny=1-\epscore(\Ny)$
(Section~\ref{sec:coordinate}) --- already computed to build $\advcoef$,
so this costs nothing extra numerically --- together with
\begin{equation}
    ...
    \qquad
    -\frac{\dot\measure}{\measure} = \frac32\dot{\Deltas}\,\yco = \frac32(1-\epscore)\,\yco ,
\end{equation}
the two $\yco$-dependent pieces combine as
\begin{equation}
    \frac32(1-\epscore)\,\yco\,X + X\,(1-\epscore)\left[\frac{1}{\Deltas}-\frac32(\yco+1)\right]
    =
    X\,(1-\epscore)\left[\frac{1}{\Deltas}-\frac32\right] ,
\end{equation}
i.e.\ the $\yco$-dependence cancels exactly, leaving the compact bracket
$(1-\epscore)[1/\Deltas-\tfrac32]$ used in \eqref{eq:inst-rphi}--\eqref{eq:inst-rpi}
above --- a single scalar per $\Ny$, cheaper to evaluate than the
$\yco$-dependent bracket it replaces, not more expensive.
\end{calculation}
...
eq. 8.1b). This separately-applied prefactor is evaluated node-by-node as
...
\begin{align}
    ...
    + \rfield_j\,\big(1-\epscore(\Ny)\big)\left[\frac{1}{\Deltas(\Ny)}-\frac32\right]
    ...
    + \rmom_j\,\big(1-\epscore(\Ny)\big)\left[\frac{1}{\Deltas(\Ny)}-\frac32\right]
    - \rfield_j + \big(3-\epsSR(\yco_j,\Ny)\big)\rmom_j ,
\end{align}
\end{subequations}
matching the corrected \eqref{eq:inst-rphi}--\eqref{eq:inst-rpi}: signs on
the advection, potential/gradient, and cross-coupling terms are flipped
relative to an earlier pass, and the bracket term's $\yco$-dependence
cancels against the (previously missing) measure-friction term as shown in
the calculation panel of Section~\ref{sec:instanton-eqs}, leaving the
single $\Ny$-dependent scalar $(1-\epscore(\Ny))[1/\Deltas(\Ny)-\tfrac32]$
above rather than a $\yco_j$-dependent vector.
