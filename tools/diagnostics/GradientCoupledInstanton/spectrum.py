...
ever sees it. This module assembles *that* operator -- the linearised map
...
this module always evaluates it at H_sq_local/H_sq_nl_init == 1 everywhere
...
deliberate simplification: the O(n_max^4) stiffness this module
...
constants (independent of phi/pi) matching this module's own frozen-
...
terms from H_sq's own state-dependence that this module's frozen
...
fallback. This module only measures.
...
Prompt 21/21a add an SBP-SAT boundary closure (`--closure sbp-sat`,
`--mode spectrum` only): split-form advection plus a dissipative SAT penalty
at the core node, replacing the plain-product/Neumann-elimination "strong"
closure now live in production. Prompt 23 extends the same assembled-operator
machinery to the response sector (which integrates backward in N, so its own
stability criterion is the negated spectral abscissa -- see the module
comment above `response_spectral_stability_metrics` below) and to a combined
forward/response adjoint-mismatch check against the SAT'd forward operator.
...
# max step this module reports is an order-of-magnitude indicator, not a
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Output CSV path (default: 'stiffness_spectrum.csv' under this "
             "package's own output/spectrum/ directory; a relative path is "
             "resolved there too, an absolute path is used as-is).",
    )
    parser.add_argument(
...
def _resolve_output_path(output: str | None) -> Path:
    """Bare/relative paths (including the default) are anchored under this
    package's own OUT_DIR, matching every sibling module's output-directory
    convention (see e.g. trajectory_plots.py's --input resolution); an
    absolute path is used as-is."""
    output = output or "stiffness_spectrum.csv"
